...
namespace http_errors {
    ...
    make_error_code(const http_error_codes e){
...
private:
    u_int m_status_code; // HTTP status code.
...
class HTTPRequest
{
    ...
    static const u_int DEFAULT_PORT = 80;

    HTTPRequest(asio::io_service & ios, u_int id)
            : m_port(DEFAULT_PORT),
              m_id(id),
              m_callback(nullptr),
              m_sock(ios),
...
public:
    ...
    void execute() {
        ...
        asio::ip::tcp::resolver::query resolver_query(m_host, std::to_string(m_port), asio::ip::tcp::resolver::query::numeric_service);
...
private:
    ...
    void on_connection_established(const boost::system::error_code& ec, asio::ip::tcp::resolver::iterator iterator)
    {
        ...
        m_request_buf += "GET ";
        m_request_buf += m_uri;
        m_request_buf += " HTTP/1.1\r\n";

        // Add mandatory header.
        m_request_buf += "Host: ";
        m_request_buf += m_host;
        m_request_buf += "\r\n";
        m_request_buf += "\r\n";
