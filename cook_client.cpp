...
public:
    ...
    std::string emulate_long_computation_op(u_int duration_sec){
...
int main_sync_tcp()
{
    // sync tcp client exmpl
    const std::string raw_ip_address = "127.0.0.1";
    const u_short port_num = 8003;
...
public:
    ...
    std::string emulate_long_computation_op(
        ...
        this->send_request(ep, request);
        std::cout << "Received..." << std::endl;
        return this->receive_response(ep);
    }

private:
    void send_request(asio::ip::udp::endpoint & ep, const std::string & request)
    {
    ...
    {
        char response[6];
        std::size_t bytes_received = m_sock.receive_from(asio::buffer(response), ep);  // may not work if buffer smaller than message from server
        std::cout << "Received: " << std::string(response, bytes_received) << std::endl;

        std::cout << "Shutdowning..." << std::endl;
//        m_sock.shutdown(asio::ip::udp::socket::shutdown_both);  // this make error for udp socket
        std::cout << "Shutdowning... done" << std::endl;
...
int main_sync_udp(){
    // sync udp client exmpl
    const std::string server1_raw_ip_address = "127.0.0.1";
    const unsigned short server1_port_num = 8003;

    const std::string server2_raw_ip_address = "127.0.0.1";
    const unsigned short server2_port_num = 8004;
    ...
    try {
        ...
        std::cout << "Sending request to the server #1 ... "
                  << std::endl;
        std::string response =
                client.emulate_long_computation_op(10, server1_raw_ip_address, server1_port_num);
        ...
        std::cout << "Sending request to the server #2... "
                  << std::endl;
        response =
                client.emulate_long_computation_op(10, server2_raw_ip_address, server2_port_num);
...
struct Session
{
    Session(asio::io_service& ios,
            const std::string& raw_ip_address, u_short port_num,
            const std::string& request, u_int id,
            Callback callback
    )
            : m_sock(ios),
              m_ep(asio::ip::address::from_string(raw_ip_address), port_num),
    ...
    u_int m_id; // Unique ID assigned to the request.
...
public:
                                     ...
                                     Callback callback, unsigned int request_id)
    {
        ...
        session->m_sock.async_connect(
                ...
                [this, session](const system::error_code& ec)
                {
                    if (ec.value() != 0) {
                        session->m_ec = ec;
                        this->on_request_complete(session);
                        return;
                    }
    ...
    void cancel_request(u_int request_id)
    {
...
void handler(u_int request_id, const std::string& response, const system::error_code & ec)
{
    if (ec.value() == 0){
...
int main()
{
    try {
        ...
        client.emulate_long_computation_op(10, "127.0.0.1", 8003, handler, 1);

        // Then does nothing for 5 seconds.
        std::this_thread::sleep_for(std::chrono::seconds(5));

        // Then initiates another request with id 2.
        client.emulate_long_computation_op(11, "127.0.0.1", 8004, handler, 2);
        ...
        client.emulate_long_computation_op(12, "127.0.0.1", 8005, handler, 3);
